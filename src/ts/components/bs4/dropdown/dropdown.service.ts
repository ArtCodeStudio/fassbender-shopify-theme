...
export class DropdownService {
  ...
  public static _clearMenus(event?: JQuery.Event | Event) {
    if (
      event &&
      ((event as JQuery.Event).which === RIGHT_MOUSE_BUTTON_WHICH ||
        (event.type === "keyup" &&
          (event as JQuery.Event).which !== TAB_KEYCODE))
    ) {
      return;
    }

    const toggles = ([].slice.call(
      $(SELECTOR.DATA_TOGGLE).get()
    ) as unknown) as HTMLUnknownElement[];
      ...
      if (
        ...
        ((event.type === "click" &&
          /input|textarea/i.test(
            ((event as Event).target as Element).tagName
          )) ||
          (event.type === "keyup" &&
            (event as JQuery.Event).which === TAB_KEYCODE)) &&
